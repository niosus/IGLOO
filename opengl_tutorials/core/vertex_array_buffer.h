...
public:
  ...
  bool EnableAttributePointer(int layout_index, bool normalized = false,
                                 void *offset = nullptr) {
    auto &bound_vertex_buffer =
        bound_buffers_[GetBoundBufferIndex(Buffer::Type::kArrayBuffer)];
    if (!bound_vertex_buffer) {
      return false;
    }
    glVertexAttribPointer(layout_index,
                          ...
                          bound_vertex_buffer->data_sizeof(), offset);
...
private:
  static inline std::size_t GetBoundBufferIndex(Buffer::Type type) {
    switch (type) {
    case Buffer::Type::kArrayBuffer:
      return 0;
    case Buffer::Type::kElementArrayBuffer:
      return 1;
    }
